 ...
 * @constructor
 */
export function PcbEditorResizeRegion(renderContext, pcb, cursor, editor, view) {
    const SPRITE_RESIZE = renderContext.getSprites().getSprite("pcbMove");
     ...
     */
    this.moveCursor = () => {
        _movable = editor.getEditable().getRegion().containsPoint(
            cursor.x * Scale.METERS_PER_POINT + editor.getEditable().getOffset().x,
            cursor.y * Scale.METERS_PER_POINT + editor.getEditable().getOffset().y);
    };
