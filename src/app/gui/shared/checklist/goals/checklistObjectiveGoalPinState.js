...
export function ChecklistObjectiveGoalPinState(goal, onDelete) {
    ...
    const makeTable = () => {
        ...
        const pinPicker = new PinPicker(goal.getPinIndex(), getPartDefinitionFromId(getPartId(goal.getPart())), selected => {
        ...
        element.appendChild(makeRow(
            ...
            new PartPicker(getPartId(goal.getPart()), selected => {
                const part = getPartDefinitionFromId(parseInt(selected));
