...
import {InputButtonCentered} from "../shared/inputButtonCentered";
...
export function MenuRoot(menu, user) {
    ...
    const make = () => {
        ...
        _element.appendChild(new InputButtonCentered(
            getString(MenuRoot.TEXT_LOAD),
            (file) => {
                menu.getGame().loadMission(file);
            }
        ).getElement());
